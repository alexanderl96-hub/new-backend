...
const {getAll , getOne, getOneFromStats, postOne, updateOne, deleteOne }= require('../queries/pitcherCareerStats');
...
router.get("/careerPitcher/:pitcher_id", async (req, res) => { 
    try {
        const statplayerID = Number(req.params.pitcher_id);
        const statSingPlayer = await getOneFromStats(statplayerID);   
        if (statSingPlayer) {
            let payload = {
              status: 'Success', 
              careerPitcher: statSingPlayer
            }
            res.json(payload);
          }else{
            throw 'Group id not found'
